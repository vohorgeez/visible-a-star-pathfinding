...
public class AStarPathfinder
{
    public List<(int x, int y)>? FindPath(Grid grid, (int x, int y) start, (int x, int y) goal)
    {
        ...
        g[start.x, start.y] = 0;
        pq.Enqueue(start, HeuristicManhattan(start.x, start.y, goal.x, goal.y));
        ...
        while (pq.Count > 0)
        {
            ...
            foreach (var n in grid.GetNeighbors4(cur.x, cur.y))
            {
                ...
                if (candidate < g[n.x, n.y])
                {
                    ...
                    int f = candidate + HeuristicManhattan(n.x, n.y, goal.x, goal.y);
                    pq.Enqueue(n, f);
                }
            }
        }
    ...
    private static int HeuristicManhattan(int x, int y, int gx, int gy)
        ...
        (int x, int y) start, (int x, int y) goal)
    {
        ...
        // If goal has no parent and isn't start, it's unreachable
